...
- The project is a Next.js 16.1.1 App Router app under `src/app`; the root layout in [src/app/layout.tsx#L1-L34] wires up the Geist and Geist Mono fonts via `next/font/google`, exports the `metadata` object, and simply renders `{children}` inside `<body>` so most structural work stays inside `src/app` subdirectories.
...
- Update the shared metadata (title/description) at [src/app/layout.tsx#L15-L18] whenever you want a new default for the entire site rather than per-page metadata overrides.

## Styling + Tailwind
- Styling lives in [src/app/globals.css#L1-L26]: it starts with `@import "tailwindcss"`, then maps `--background`/`--foreground` to Tailwind tokens via `@theme inline`, and finally sets the body background, font stack, and a dark-mode override using `prefers-color-scheme`. Adjust the CSS custom properties here whenever you need new palette entries for `text-[var(--foreground)]`, `bg-[var(--background)]`, etc.
- Because this template uses Tailwind 4, there is no `tailwind.config.js`; the build pipeline comes directly from [postcss.config.mjs#L1-L6], which instantiates the `@tailwindcss/postcss` plugin to process whatever lands in `globals.css`.
- Keep all team-wide utility tweaks (colors, font variables, dark-mode media queries) inside `globals.css` to ensure every page inherits the same tokens without duplicating `@tailwind` directives.
...
## Components
- The shared `Header` component in [src/components/layout/Header.tsx](../src/components/layout/Header.tsx) displays navigation links to all 6 main categories and includes a cart button with badge showing total item count. It uses `useCartStore()` to track cart state and `toggleCart()` to open the drawer.
- The shared `Footer` component in [src/components/layout/Footer.tsx](../src/components/layout/Footer.tsx) includes Quick Links (Home, About, Contact), Categories (Men, Women, Unisex, Pro, Eco), Legal links (Privacy, Terms, Refund, Shipping), and Social media icons (Facebook, Instagram, WhatsApp, TikTok).
